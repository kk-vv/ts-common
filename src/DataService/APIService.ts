...
export const APIService = {
  ...
  async request(target: APIRequestTarget, withToken: boolean = true) {
  ...
  }, async fetch<T>(target: APIRequestTarget, withToken: boolean = true) {
    try {
      const result = (await this.request(target, withToken)) as APIDataResponse<T>
      if (result.data !== null) {
  ...
  }, async fetchNullable<T>(target: APIRequestTarget, withToken: boolean = true) {
    try {
      const result = (await this.request(target, withToken)) as APIDataResponse<T>
      return result.data
    } catch (error) {
